...
from notemonkey.word_segmenter.connected_component_segmenter import ConnectedComponentWordSegmenter
from  notemonkey.model.basemodel import BaseModel

class TestDPWordChunkSegmenter():
    segmenter = ConnectedComponentWordSegmenter(BaseModel())
    
    def test_on_1by1(self):
        """we expect to get no characters here"""
        one_by_one = np.array([[255]], np.uint8)
    ...
    def test_on_image_no_text(self):
        input = (np.ones((100, 300))*255).astype(np.uint8)
        output = self.segmenter.segment(input)
        assert output == []

    def test_on_all_black(self):
        input = np.zeros((100,300)).astype(np.uint8)
        output = self.segmenter.segment(input)
        assert len(output) == 1 and output[0][0].image.shape == input.shape and np.all(output[0][0].image == input)
